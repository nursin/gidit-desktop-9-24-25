...
electron_1.ipcMain.handle('db:getTasks', () => {
...
electron_1.ipcMain.handle('db:addTask', (_event, task) => {
    const tasks = store.get('tasks');
...
electron_1.ipcMain.handle('db:updateTask', (_event, task) => {
    const tasks = store.get('tasks');
...
// Delete a task by ID
electron_1.ipcMain.handle('db:deleteTask', (_event, id) => {
    const tasks = store.get('tasks');
...
// unavailable.
electron_1.ipcMain.handle('db:query', () => {
