...
require$$1$2.ipcMain.handle("db:addTask", (_event, task) => {
  ...
  const newTask = { id: id2, title: task.title, status };
...
async function ensureResources() {
  ...
  if (!fs$3.existsSync(modelsDir)) {
    fs$3.mkdirSync(modelsDir, { recursive: true });
    fs$3.writeFileSync(
      path$6.join(modelsDir, "README.txt"),
      "This folder should contain the gemma3-270m model files.\n"
    );
    console.log("Created placeholder model directory at", modelsDir);
  }
  if (!fs$3.existsSync(pythonRuntimeDir)) {
    fs$3.mkdirSync(pythonRuntimeDir, { recursive: true });
    fs$3.writeFileSync(
      path$6.join(pythonRuntimeDir, "README.txt"),
...
function loadAppIcon() {
  ...
  const searchRoots = [
    ...
    path$6.join(appRoot, "build/icons"),
    path$6.resolve(appRoot, "..", "build/icons")
  ...
  for (const root of searchRoots) {
    for (const candidate of iconCandidates) {
      ...
      try {
        ...
        console.warn(`[icon] failed to load ${absolute}:`, error2);
      }
    }
  }
